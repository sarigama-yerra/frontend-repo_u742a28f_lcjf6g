        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-black/30 backdrop-blur-sm flex items-center justify-center p-4">
          <motion.div initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 40, opacity: 0 }} className="bg-white rounded-2xl shadow-2xl w-full max-w-xl overflow-hidden">
            ...
            <div className="p-4 grid gap-4">
              ...
              {step === 1 && (
                <form onSubmit={() => setStep(2)} className="grid gap-3">
                  ...
                  <button className="mt-2 bg-blue-600 text-white rounded-lg py-2 font-medium">Continue to payment</button>
                </form>
              )}
                ...
                <form onSubmit={pay} className="grid gap-3">
                  ...
                  <div className="flex gap-3">
                    {['razorpay','cashfree','stripe'].map(g => (
                      <button type="button" key={g} onClick={() => setGateway(g)} className={`px-4 py-2 rounded-lg border ${gateway===g?'border-blue-600 bg-blue-50 text-blue-700':'border-slate-200'}`}>{g}</button>
                    ))}
                  </div>
                  <button className="mt-2 bg-emerald-600 text-white rounded-lg py-2 font-medium">Pay ₹{amount}</button>
